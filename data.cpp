...
DATA::DATA ( QString data, int id){
    ...
    if(id == 1){//ru
        alphabet.append("АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя1234567890.,:;<>+-=_/?!'\"|\\~`#$%^&*(){}[]\ \t\n");
    }
    if(id == 2){//uk
        alphabet.append("АБВГҐДЕЄЖЗИІЇЙКЛМНОПРСТУФХЦЧШЩЬЮЯабвгґдеєжзиіїйклмнопрстуфхцчшщьюя1234567890.,:;<>+-=_/?!'\"|\\~`#$%^&*(){}[]\ \t\n");
    }
    if(id == 3){
